...
class SerialNoSteer:
    ...
    def switch_mode(self):
        ...
        if(self.is_idle== True and self.buttons[2]):
        ...
        if(self.choose_mode and self.buttons[4]):
            self.is_drive = True
            self.choose_mode = False
            self.is_idle = False
            self.is_science = False
    ...
    def wheel_speed_calc(self):
        if(self.lineer_x != None and self.angular_z != None ):
            ...
            self.wheel_speeds.data = [self.rpm_left*2,self.rpm_right*2, self.torq_mode]
                
            
            
            self.wheel_speeds.data = [ self.rpm_left, self.rpm_right, self.torq_mode]
