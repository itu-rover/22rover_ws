...
class Approach_by_loc:
    def __init__(self):
        ...
        self.starting_point = (1.5,0)  # starting point 
        self.start_odom = np.array((self.x,self.y)) # holding starting odometry
        self.goal_number = 0 # goal number 
        self.spihiral_angle =  m.pi/6 # rotated angle in every step 
        self.starting_angle = self.spihiral_angle  # yaw of first goal 
        self.sphiral_points(1.05,1.08,self.spihiral_angle) # found sphiral points since sphiral grows exponentially at the beginning it grows with bigger r then after a point it will use smaller r // 1.1,1.15
    ...
    def yolo(self,data):
        if(len(self.count_tag) == 0):
            ...
            if(self.yolo_output == "okay") and (self.flag_yolo_okey == False):
                self.send_goal(2,0,0,"base_link",wait_flag=False)
    ...
    def run(self):
        while (not rospy.is_shutdown()):
            if self.initialized == False and self.starting_angle != None:
                self.go_on()
                self.send_goal(self.x + self.starting_point[0],self.y + self.starting_point[1],self.starting_angle,wait_flag=  False)
            ...
            if len(self.ar_tags) != 0:
                ...
                self.go_on()
                #self.send_goal(0,0,m.atan2(marker_y,marker_x),"base_link")
                marker_x,marker_y = list(self.ar_tags.values())[0].translation.z,-list(self.ar_tags.values())[0].translation.x
                #if(marker_x-5 > 0):
                    #self.send_goal(marker_x-5,0,0,"base_link")
                return True
            self.Rate.sleep()
    ...
    def draw_sphiral(self):
        ...
        self.send_goal(x,y,m.atan2(y_n-self.y,x_n-self.x)+0.5,wait_flag= False)
    ...
    def turn_around(self,angular_speed):
        ...
        rospy.loginfo("donuyoz aruco bulamadik")
        self.hand_drive.publish(t)

        if((rospy.Time.now().secs -  self.starting_look_around > 1) and abs(self.yaw - self.initial_yaw) < 0.2):
    ...
    def send_goal(self,x, y, yaw, frame_id='odom',wait_flag = True):
